...
- 1. Render personal info, 12 group cards (drag-drop), thirds picker, bracket, extras
- 1. Track completion state → enable submit button when all required
- 1. Submit JSON → show confirmation
   */
...
return `<header> <div class="brand">VM<span class="brand-accent">26</span></div> <div class="tagline">Tipping · Canada · Mexico · USA</div> </header>`;
}
...
return `<div class="deadline-strip ${open ? '' : 'closed'}" id="deadline-strip"> <span>${open ? 'FRIST' : 'STENGT'}</span> <span class="deadline-countdown" id="countdown">${d ? formatDeadline(d) : '—'}</span> </div>`;
}
...
return `<section class="section" id="sec-personal"> <div class="section-num">01 · Deltaker</div> <h2 class="section-title">Hvem er du?</h2> <p class="section-desc">Alle felt er obligatoriske. E-postadressen brukes for å unngå duplikate innleveringer.</p> <div class="field"> <label for="field-full_name">Fullt navn <span class="req">*</span></label> <input type="text" id="field-full_name" placeholder="Fornavn Etternavn" autocomplete="name"> </div> <div class="field"> <label for="field-email">E-post <span class="req">*</span></label> <input type="email" id="field-email" placeholder="navn@firma.no" autocomplete="email"> </div> <div class="two-col"> <div class="field"> <label for="field-company">Arbeidssted <span class="req">*</span></label> <input type="text" id="field-company" placeholder="f.eks. Oslo" autocomplete="organization"> </div> <div class="field"> <label for="field-department">Avdeling <span class="req">*</span></label> <input type="text" id="field-department" placeholder="f.eks. Equity Research"> </div> </div> </section>`;
}
...
const items = teams.map((t, i) => `<li class="team-item" data-team="${t.team_code}"> <span class="pos-badge">${i + 1}</span> <span class="team-flag">${teamFlag(t.team_code)}</span> <span class="team-name">${escapeHTML(t.name_no || t.name_en)}</span> <span class="drag-handle">⋮⋮</span> </li>`).join(’’);
return `<div class="group-card"> <div class="group-card-header"> <span class="group-label">Gr. ${g}</span> <span class="group-hint">Dra for å rangere</span> </div> <ul class="team-list" id="group-list-${g}">${items}</ul> </div>`;
}).join(’’);
...
return `<section class="section"> <div class="section-num">03 · Beste 8 treere</div> <h2 class="section-title">Hvilke 8 av 12 treere går videre?</h2> <p class="section-desc">De 8 beste 3.-plassene går til R32. Velg hvilke 8 av dine predikerte 3.-plasser du tror går videre.</p> <div id="thirds-container">${renderThirdsInner()}</div> </section>`;
}
...
return `<div class="third-chip ${isSel ? 'selected' : ''}" data-team="${thirdCode}" onclick="toggleThird('${thirdCode}')"> <span class="grp-letter">${g}</span> <span class="team-flag" style="font-size:0.9rem;">${team ? teamFlag(thirdCode) : ''}</span> <span>${team ? escapeHTML(team.name_no || team.name_en) : '—'}</span> </div>`;
}).join(’’);
const count = state.prediction.thirds.length;
return `<div class="thirds-counter ${count === 8 ? 'full' : ''}">VALGT ${count} / 8</div> <div class="thirds-grid">${chips}</div>`;
}
...
return `<section class="section"> <div class="section-num">04 · Sluttspill</div> <h2 class="section-title">Bygg din bracket</h2> <p class="section-desc">Klikk på laget du tror vinner hver kamp. Vinnerne går videre til neste runde.</p> <div id="bracket-container">${renderBracketInner()}</div> </section>`;
}
...
return `<div class="matchup"> <div class="match-team ${homeClass}" ${home ?`onclick=“pickWinner(${id}, ‘${home}’, ‘${round}’)”`: ''}> <span class="match-id">#${id}</span>${homeLabel} </div> <div class="match-vs">VS</div> <div class="match-team ${awayClass}" ${away ?`onclick=“pickWinner(${id}, ‘${away}’, ‘${round}’)”`: ''}> ${awayLabel} </div> </div>`;
}
...
return `<div class="question"> <div class="question-meta"> <span>#${q.q_id}</span> <span class="pts">${q.points}p</span> <span>${q.round}</span> </div> <div class="question-text">${escapeHTML(txt)}</div> ${input} </div>`;
}).join(’’);
return `<section class="section"> <div class="section-num">05 · Ekstraspørsmål</div> <h2 class="section-title">13 bonus-spørsmål</h2> <p class="section-desc">Totalt 37 ekstrapoeng å hente. Svar på alle før du leverer.</p> ${questions} </section>`;
}

// ── Submit bar ───────────────────────────────────────────────
function renderSubmitBar() {
return `<div class="submit-bar"> <div class="completion-text"> <span id="completion-status">Fyller ut…</span> </div> <button type="button" class="btn-submit" id="btn-submit" disabled onclick="submitForm()"> Lever </button> </div>`;
}
...
document.getElementById(‘app’).innerHTML = `<div class="confirmation"> <div style="font-size:3rem; margin-bottom:1rem;">⚽</div> <h2>Takk, ${escapeHTML(state.prediction.personal.full_name)}!</h2> <p style="color:var(--muted);">Din innlevering er mottatt og registrert.</p> <div class="confirmation-code">${resp.confirmation_code}</div> <p style="font-size:0.875rem; color:var(--muted);"> Ta skjermbilde av koden for referanse.<br> Resultater publiseres fortløpende på rapportsiden. </p> <p style="margin-top:2rem; font-family:'JetBrains Mono',monospace; font-size:0.8125rem;"> 11. juni 2026 → 19. juli 2026 </p> </div>`;
window.scrollTo(0, 0);
}
